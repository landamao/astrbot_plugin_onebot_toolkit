...
| `get_group_msg_history` | 获取当前群聊近 n 条消息记录，格式化为易读的对话记录（默认 20 条，上限 100） |
...
- **参数**：
  ...
  - `max_length` (number, 可选)：每条消息内容的最大字符数，超出截断用省略号。默认 100，传 -1 不截断
- **返回**：格式化的发言记录，每行格式为 `[message_id] 消息内容`，按时间倒序排列
